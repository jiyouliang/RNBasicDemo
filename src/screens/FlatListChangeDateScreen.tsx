...
export default class FlatListChangeDateScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [
  ...
  _selectItemPress = item => {
    if (item.select) {
      this.state.selectItem.splice(
        this.state.selectItem.findIndex(function(x) {
          return x === item.name;
        }),
      );
    } else {
    ...
    return (
      <FlatList
        ...
        ListHeaderComponent={({item}) => {
          return (
            <Button title={'确定'} onPress={(_ = () => this._submitPress())} />
          );
        }}
        renderItem={({item}) => {
          return (
            <TouchableHighlight onPress={_ => this._itemPress(item)}>
              <View style={styles.standaloneRowFront}>
                <Image
                  style={{width: 70, marginLeft: 5, height: 70}}
